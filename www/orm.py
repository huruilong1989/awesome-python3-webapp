...
class ModelMetaclass(type):
    def __new__(cls, name, bases, attrs):
        ...
        attrs['update__'] = "update '%s' set %s where '%s'=?" % (
...
class Model(dict, metaclass=ModelMetaclass):
    ...
    async def find(cls, pk):
        ...
        rs = await select("%s where '%s'=?" % (cls.select__, cls.__primary_key__), [pk], 1)
        if len(rs) == 0:
            return None
        return cls(**rs[0])
    ...
    async def save(self):
        ...
        rows = await execute(self.__inser__, args)
        if rows != 1:
